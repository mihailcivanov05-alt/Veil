...
- **2026-09-09 v3.4 — reels shared in a DM:** On-device the leaking reel scroll was a reel *shared into a DM*: tapping it opens a fullscreen vertical video feed while the URL stays `/direct/t/{id}/`. `/direct/` is hard-whitelisted (Invariant 2) so nothing engaged. Diag bookmarklet on-device showed 3+ stacked `402x644` `<video>`s in a 714 viewport. `checkDmReelViewer()` (run from `sweep()` + end of `handleInstagram()`) arms `state.isSharedReelActive` when ≥2 near-fullscreen videos (h>70%/w>60% of viewport) sit on a `/direct/` route — a shape ordinary messaging cannot make, so DMs stay untouched. Reuses the v3.3 capture-phase touchmove lock; blackout returns to the captured thread URL, not the inbox. `handleInstagram()` no longer clears `isSharedReelActive` while `dmReelArmed`. `node --check` OK.
- **2026-09-09 v3.5 — pin the DM feed scroller:** On-device HUD (debug bookmarklet) showed the DM-shared reel opens IG's fullscreen **"Suggested"** feed on `/direct/t/{id}/`, 15 stacked `402x644` `<video>`s, scrolling a native `overflow` `<div>` (`scrollTop` 4550→6440 while swiping). `preventDefault()` on touch does not stop a native scroll container on iOS. v3.5: `pinDmScrollers()` walks every scrollable ancestor of the on-screen fullscreen video and sets `overflow:hidden` + `touch-action:none !important` (`data-veil-pinned`), re-applied each `sweep()` while armed; capture-phase `scroll` listener snaps `scrollTop` back to the locked value and blackouts (→ thread URL) if it moves >40px anyway. Detection relaxed 2→1 on-screen fullscreen video, with viewport-intersection gate so it releases + unpins when the viewer closes. `node --check` OK.
- **2026-09-09 v3.5.1 — in-script telemetry + guaranteed poll:** 5 blind DM-reel iterations with no on-device console. Added `stampDbg()` → writes `data-veil-v` / `data-veil-dbg` (armed/page/sra/dm/pin/black) onto `<html>` every `checkDmReelViewer()`, plus `window.__VEIL` {status, recheck, scan} for a HUD bookmarklet to read regardless of sandboxing. Also a `setInterval(checkDmReelViewer, 600)` on IG so arming no longer depends on the MutationObserver firing. Next step gated on reading the HUD on-device: confirms whether the fresh script is even loading (`data-veil-v`), and if so why the pin/scroll-lock is not catching the DM feed.
